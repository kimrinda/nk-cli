/**
 * @file HTTP-mode series detail scraper for hanimeindex items.
 *
 * Reads the A–Z index from `output/hanimeIndex.json`, iterates every
 * item, fetches the series page at `<host>/hentai/<slug>/`, parses it
 * with the Cheerio-based {@link parseSeriesPageHtml}, and writes the
 * result back into the index JSON under `groups.X.items[N].details`.
 *
...
import { parseSeriesPageHtml } from '../parsers/cheerio/seriesPage.js';
...
/**
 * Scrape series details for every item in the hanimeindex via HTTP.
 *
 ...
 */
export async function scrapeAzDetailsHttp(session, indexPath, options = {}) {
  /** @type {AzIndex} */
  ...
  try {
    ...
    for (let i = startIndex; i < refs.length; i += 1) {
      ...
      try {
        const html = await withRetry(() => session.fetchHtml(ref.url), {
          attempts: config.scrape.retryAttempts,
          baseDelayMs: config.scrape.retryBaseDelayMs,
          label: `fetchSeriesPage(${ref.slug})`,
        });

        const details = parseSeriesPageHtml(html, ref.url);
        ...
        logger.info('Series detail scraped (HTTP)', {
          slug: ref.slug,
          group: ref.groupKey,
          index: i + 1,
          total: refs.length,
        ...
        failed += 1;
        logger.error('Series detail scrape failed (HTTP), continuing', {
          slug: ref.slug,
          url: ref.url,
    ...
    logger.info('AZ detail scrape complete (HTTP)', {
      total: refs.length,
      processed,
      skipped,
      failed,
